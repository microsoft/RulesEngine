...
public class TypedClassTests
{
    ...
    public async Task TypedClassTest()
    {
        ...
        var reSettings = new ReSettings { CustomTypes = new Type[] { }, AutoRegisterInputType = false };
        var re = new RulesEngine(reSettings);
        re.AddWorkflow(workflow);

        var param = new Transazione {
            Attori = new List<Attore> { new() { RuoloAttore = RuoloAttore.B }, new() { RuoloAttore = RuoloAttore.C } }
        };
    ...
    public async Task TypedClassInputSameNameAsTypeTest()
    {
        ...
        var reSettings = new ReSettings { CustomTypes = new[] { typeof(Transazione) } };
        var re = new RulesEngine(reSettings);
        re.AddWorkflow(workflow);
    ...
    public async Task TypedClassBothAccessibleTestWhenCaseInsensitive()
    {
        Workflow workflow = new() {
            ...
            Rules = new Rule[] {
                ...
                new() { RuleName = "Static FieldTest", Expression = "Transazione.StaticProperty == \"Hello\"" }
            }
        };
        var reSettings = new ReSettings {
            CustomTypes = new[] { typeof(Transazione) }, IsExpressionCaseSensitive = true
        };
        var re = new RulesEngine(reSettings);
        re.AddWorkflow(workflow);
