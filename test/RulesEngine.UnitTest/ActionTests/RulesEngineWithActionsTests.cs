...
public class RulesEngineWithActionsTests
{
    ...
    public async Task WhenExpressionIsSuccess_OutputExpressionAction_ReturnsExpressionEvaluation()
    {
        ...
        var result =
            await engine.ExecuteActionWorkflowAsync("ActionWorkflow", "ExpressionOutputRuleTest", new RuleParameter[0]);
    ...
    public async Task WhenExpressionIsSuccess_ComplexOutputExpressionAction_ReturnsExpressionEvaluation()
    {
        var engine = new RulesEngine(GetWorkflowWithActions());
        var result =
            await engine.ExecuteActionWorkflowAsync("ActionWorkflow", "ComplexOutputRuleTest", new RuleParameter[0]);
        Assert.NotNull(result);
    ...
    public async Task WhenExpressionIsSuccess_EvaluateRuleAction_ReturnsExpressionEvaluation()
    {
        var engine = new RulesEngine(GetWorkflowWithActions());
        var result =
            await engine.ExecuteActionWorkflowAsync("ActionWorkflow", "EvaluateRuleTest", new RuleParameter[0]);
        Assert.NotNull(result);
        Assert.Equal(2 * 2, result.Output);
    ...
    public async Task ExecuteActionWorkflowAsync_CalledWithIncorrectWorkflowOrRuleName_ThrowsArgumentException()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(async () =>
            await engine.ExecuteActionWorkflowAsync("WrongWorkflow", "ExpressionOutputRuleTest", new RuleParameter[0]));
        await Assert.ThrowsAsync<ArgumentException>(async () =>
            await engine.ExecuteActionWorkflowAsync("ActionWorkflow", "WrongRule", new RuleParameter[0]));
    }
    ...
    {
        var engine = new RulesEngine(GetWorkflowWithActions());
        var result = await engine.ExecuteActionWorkflowAsync("WorkflowWithGlobalsAndSelfRefActions",
            "RuleReferencingSameWorkflow", new RuleParameter[0]);
        Assert.NotNull(result);
        Assert.Null(result.Output);
    }
    ...
    public async Task ExecuteActionWorkflowAsync_SelfReferencingAction_WithFilter_ExecutesSuccessfully()
    {
        var engine = new RulesEngine(GetWorkflowWithActions());
        var result = await engine.ExecuteActionWorkflowAsync("WorkflowWithGlobalsAndSelfRefActions",
            "RuleReferencingSameWorkflowWithInputFilter", new RuleParameter[0]);
        Assert.NotNull(result);
    ...
    private Workflow[] GetWorkflowWithActions()
    {
                ...
                new() {
                    ...
                    Actions =
                        new RuleActions {
                            OnSuccess = new ActionInfo {
                                Name = "OutputExpression",
                                Context = new Dictionary<string, object> { { "expression", "new (2 as test)" } }
                            }
                        }
                },
                new() {
                    ...
                    Actions = new RuleActions {
                        OnSuccess = new ActionInfo {
                            ...
                            Context = new Dictionary<string, object> {
                                { "workflowName", "ActionWorkflow" }, { "ruleName", "ExpressionOutputRuleTest" }
                            }
                        }
                    }
                }
        ...
        var workflow2 = new Workflow {
            ...
            GlobalParams = new[] { new ScopedParam { Name = "global1", Expression = "\"Hello\"" } },
            Rules = new[] {
                new Rule {
                        ...
                        OnSuccess = new ActionInfo {
                            ...
                            Context = new Dictionary<string, object> {
                                ...
                                { "inputFilter", new string[] { } }, {
                                    "additionalInputs",
                                    new[] { new ScopedParam { Name = "additionalValue", Expression = "1" } }
                                }
                            }
                        }
                    }
